...
For example, we can reproduce the table exchangeable correlations on page 29 of \cite{diggle} for the case of $\bm{t} = (0,2,5)'$, $\alpha=0.05$, power=0.80, and $d=0.5$ as follows via the \texttt{\hlkwd{diggle.linear.power}} function:
<<>>=
n = 3
t = c(0,2,5)
rho = c(0.2, 0.5, 0.8)
